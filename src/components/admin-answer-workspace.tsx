...
function AnswerComposer({
  ...
  return (
    <div className="answer-composer">
      ...
      {aiDraft && <details className="answer-composer__ai" open={draftSources.length > 0}><summary><Bot size={14} /> Review AI draft {draftConfidence && <span className={`draft-confidence draft-confidence--${draftConfidence}`}>{draftConfidence} confidence</span>}{question.ai_answer_mode && <span className={`draft-mode draft-mode--${question.ai_answer_mode}`}>{question.ai_answer_mode === "course" ? "Course-grounded" : "External knowledge"}</span>}</summary><div>{aiDraft}</div>
